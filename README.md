...
> 开源的零渲染、秒级多机位音频与画面同步神器。保护本地隐私，无需上传视频到云端。
...
MatchCut 是一款面向剪辑师与视频创作者的本地桌面端工具：
- **精准帧匹配**：基于帧相似度算法（特征比对），自动在原始素材中找到参考视频对应的时间码。
- **一键导出草稿**：直接导出 `.jyproj` 剪映草稿或标准 FCPXML，导入剪辑软件后时间轴与片段自动排好，告别手动拉片。
- **本地沙盒运行**：纯本地 CPU/GPU 运算，**绝不上传任何隐私素材到云端**。
- **多语言与跨平台**：支持中文、English、Español、Deutsch，完美适配 macOS (Apple Silicon / Intel) & Windows。

---

## 💻 运行与使用

1. 前往 [GitHub Releases](https://github.com/Joe651616165/MatchCut/releases) 下载对应系统的最新安装包（如 Windows 的 `.exe` 或 macOS 的 `.dmg`）。
2. 打开软件，加载【参考成片】与【原始素材】。
3. 设置相似度阈值并开始匹配，预览结果无误后导出剪映工程。

---

## ⚠️ 免责声明

本工具仅作为技术研究与效率提升辅助之用。请用户确保对所处理的视频素材拥有合法版权，切勿用于侵权盗版行为，否则产生的一切后果由使用者自行承担。
